...
export class AppComponent implements OnInit { 
  ...
  async searchCarSubmit() { 
    if (this.form_search.valid) { 
      ...
      this.queryStr = this.form_search.value.year + ' ' + 
                      this.form_search.value.make + ' ' + 
                      this.form_search.value.model + ' ' + 
                      this.form_search.value.part; 
      ...
      if (!this.suggestions) { 
        ...
        if (this.form_search.valid) { 
          ...
          this.subscription = this.dataService.posteBay( 
            this.form_search.value.year, 
            this.form_search.value.make, 
            this.form_search.value.model, 
            this.form_search.value.part, 
            this.form_search.value.suggest 
          ).subscribe( 
            data => { 
              ...
              if (data['message'] == 'end of stream') { 
                ...
                let uniqueList = this.comparisonData.filter((obj, index, self) =>
                index === self.findIndex((o) => o.title === obj.title && o.comp === obj.comp)
              );
